...
private const val CACHE_LOG_TAG = "ThreeTierCache"
 ...
 */
class ThreeTierCache<Key, Value>(
    ...
    fun observe(key: Key): Flow<Value> = flow {
        ...
        if (fallback != null) {
            Logger.d(CACHE_LOG_TAG) { "内存命中 key=$key" }
            emit(fallback)
        }
        ...
        if (local != null) {
            ...
            Logger.d(CACHE_LOG_TAG) { "本地命中 key=$key" }
            emit(local)
        }
        try {
            Logger.d(CACHE_LOG_TAG) { "请求远程 key=$key" }
