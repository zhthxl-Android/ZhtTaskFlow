...
private const val DATA_STORE_LOG_TAG = "AppDataStore"
...
/** 写入 String 偏好（IO 线程，失败记录日志并抛出 [AppIllegalStateException]） */
...
private suspend fun DataStore<Preferences>.runDataStoreIo(
    ...
    withContext(Dispatchers.IO) {
        try {
            ...
            throw AppIllegalStateException(message = "本地偏好存储失败", cause = io)
        } catch (throwable: Throwable) {
            logDataStoreFailure("DataStore 操作失败: $operation", throwable)
            throw AppIllegalStateException(message = "本地偏好存储失败", cause = throwable)
        }
    }
}

private fun logDataStoreFailure(summary: String, throwable: Throwable) {
    Logger.d(DATA_STORE_LOG_TAG, throwable) { summary }
}
