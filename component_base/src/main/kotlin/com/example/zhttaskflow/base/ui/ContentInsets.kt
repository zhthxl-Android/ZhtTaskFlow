 ...
 *   适合弹窗表单、短内容区。
 */
enum class ImeAvoidanceMode {
    BottomPadding,
    BringIntoView,
}

/**
 * [rememberImePadding] 的聚合结果：padding + 容器 Modifier，业务一次解构即可套用。
 */
@Immutable
data class ImePaddingState(
    val padding: PaddingValues,
    val contentModifier: Modifier,
)

@Composable
fun rememberScaffoldContentPadding(
...
/**
 * [StateBox] 四态（Loading/Empty/Error）内容区 padding：列表页水平边距由 [rememberListLazyContentPadding] 承担，
 * 此处为零，避免与列表成功态双重水平 inset（与 [com.example.zhttaskflow.feature.log.presentation.LogScreen] 一致）。
 */
@Composable
fun rememberStateBoxContentPadding(): PaddingValues = PaddingValues(0.dp)

/**
 * 沉浸式头部内避开状态栏的 top inset（Banner 图可全屏铺到顶，文字/按钮用此 padding）。
 */
@Composable
fun rememberStatusBarTopInset(): Dp {
...
/**
 * 统一 IME 键盘避让：与 [InsetsPolicy]（Scaffold 不消费 IME）配合，由业务内容区按需调用。
 *
 * ```
 ...
 */
@Composable
fun rememberImePadding(
    mode: ImeAvoidanceMode = ImeAvoidanceMode.BottomPadding,
    extraBottom: Dp = 0.dp,
): ImePaddingState {
    val imeBottom = rememberImeBottomInsetDp()
    val scrollState = rememberScrollState()
    return when (mode) {
        ImeAvoidanceMode.BottomPadding -> {
        ...
        ImeAvoidanceMode.BringIntoView -> {
 ...
 */
@Composable
fun rememberImeBottomInsetDp(): Dp {
    val density = LocalDensity.current
    return with(density) {
...
/**
 * 将 [rememberImePadding] 的结果应用到 Modifier（先 padding 再容器滚动等行为）。
 */
fun Modifier.imePadding(state: ImePaddingState): Modifier {
    return this
