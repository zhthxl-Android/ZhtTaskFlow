...
/**
 * Feature 模块禁止直接调用 [android.widget.Toast]，统一走壳层 [com.example.zhttaskflow.base.ui.BaseScaffold] Snackbar。
 */
class NoToastInFeatureDetector : Detector(), SourceCodeScanner {
        ...
        override fun visitCallExpression(node: UCallExpression) {
            ...
            if (containingClass == "android.widget.Toast") {
                context.report(
                    ...
                    node,
                    context.getLocation(node),
                    "Feature 层禁止直接调用 Toast API（${method.name}），请使用全局 Snackbar（BaseScaffold / UiEffect）。",
                )
            }
        }
    }
...
- 由 `BaseScaffold` 的 `SnackbarHost` 统一展示；
- 需要阻塞反馈时使用 `BlockingLoadingOverlay` 或 Dialog，而非 Toast。
"""

        val ISSUE: Issue = Issue.create(
            id = LintIds.NO_TOAST_IN_FEATURE,
            ...
            implementation = Implementation(
                NoToastInFeatureDetector::class.java,
